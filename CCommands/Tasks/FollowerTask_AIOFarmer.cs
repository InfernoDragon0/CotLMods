...
{
    internal class FollowerTask_AIOFarmer : CustomTask
    {
        public override string InternalName => "FollowerTask_AIOFarmer";
        private int _resourceStationID;
        private Structures_AIOFarmer _resourceStation;
        ...
        public FollowerTask_AIOFarmer(int resourceStationID)
        {
            this._resourceStationID = resourceStationID;
            this._resourceStation = StructureManager.GetStructureByID<Structures_AIOFarmer>(this._resourceStationID);
        }

        public FollowerTask_AIOFarmer()
        {
            foreach (StructureBrain structureBrain in StructureManager.StructuresAtLocation(FollowerLocation.Base))
            {
                if (structureBrain is Structures_AIOFarmer && !structureBrain.ReservedForTask)
                {
        ...
        {
            base.Setup(follower);
            follower.SimpleAnimator.ChangeStateAnimation(StateMachine.State.Idle, "Farming-water");
            Plugin.Log.LogInfo("the follower " + follower.Brain._directInfoAccess.Name + " is working on a AIO farm");
        }
